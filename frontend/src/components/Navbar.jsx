            ...
            <div className="container mx-auto px-4 py-2 flex justify-between items-center">
                ...
                <div className="hidden lg:flex space-x-6 flex-grow justify-end">
                    ...
                    {Cookies.get('token') ? (<>
                        ...
                        <button className="btn btn-ghost text-green-400" onClick={() => { toast.success(`token: ${Cookies.get('token')}`); Cookies.set('token', ''); Cookies.remove('token', { path: '/', domain: '.blog.jules.tf' }); console.log('removing cookie token with path / and domain .blog.jules.tf'); localStorage.clear(); navigate('/'); toast.success('Successfully logged out!', { position: 'bottom-right' }) }} key={"logout"}><MdOutlineAdminPanelSettings className='inline align-middle' /> <span className='align-middle'>Logout</span></button>
                    </>) :
